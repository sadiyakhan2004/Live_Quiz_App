...
function Home() {
  ...
  const [roomName, setRoomName] = useState<string | string[]>(quizId);
  ...
  useEffect(() => {
    const checkExistingSession = () => {
      ...
      if (sessionData && userId) {
        ...
        if (parsedSession.roomId === quizId) {
  ...
  const handleUserAuth = async ({
    ...
    try {
      // First fetch quiz data
      const quizData = await fetchQuiz(quizId);
      const questionsData = await fetchQuestions(quizData.quizName);
      console.log("Fetched quiz data:", quizData);
      setQuiz(quizData);
      setQuestions(questionsData);

      // Then proceed with room joining
  ...
  const roomRejoin = (username: string, email: string): void => {
    socket?.emit(
      "room-rejoin",
      { storedSession, userId, roomName: quizId },
      async (data: {
        rtpCapabilities: any;
        isAdmin: boolean;
  ...
  const createDevice = async (rtpCapabilities: any): Promise<Device> => {
    try {
      ...
      deviceRef.current = device; // Store the device instance in a ref for later use
    ...
    try {
      ...
      const checkActive = () => {
        if (videoTrack.readyState === "ended") {
          ...
          socket?.emit("stop-screen-share", { roomName });
        } else {
  ...
  const stopScreenSharing = async (): Promise<void> => {
    ...
    try {
      ...
      if (socket) {
        console.log(
          "Emitting stop-screen-share event from track ended for room:",
          roomName
        );
        socket.emit("stop-screen-share", { roomName });
      }
